...
class RegressionRunner : Instrumentation() {
    ...
    override fun onStart() {
        try {
            ...
            checkHome()
            checkPeak()
            finish(Activity.RESULT_OK, Bundle().apply { putString("stream", "PASS: notification activity/cancelled/non-activity launches, inline reply retention and swipe dismissal; note focus/title/scroll/reorder; todo back/reorder; notifier gating/gestures; 90% font; status insets; weather configuration\n") })
        } catch (e: Throwable) {
