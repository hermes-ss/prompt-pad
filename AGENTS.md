...
- New blank notes focus the title; existing notes open with the body focused and cursor at position zero. The `<>` toolbar command inserts a backtick pair at the selection and puts the cursor between them. View/edit transitions preserve a source-line scroll anchor across markdown rendering; Back closes the note before leaving Notes. Notes and tasks use explicit up/down reorder controls, persisted through the existing store; Notes keeps reorder at the right of New Note, while To Do keeps it at the left of Clear. Orange scrollbar thumbs appear for overflowing lists and note bodies. No drag-and-drop dependency.
...
- Signed/minified release installs and cold-launches without an AndroidRuntime crash. APK Signature Scheme v2 verification passes, the manifest is non-debuggable, and SHA-256 is `5e68aca282d9c696634609cd5f2521c36eb676bd722ecb334cecb623abe67cb2`. Artifact: `app/build/outputs/apk/release/prompt-pad-release.apk`.
